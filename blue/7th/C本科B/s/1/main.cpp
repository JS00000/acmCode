...
// int checkDfs(int x, int y, bool mp[][4],int deep)
// {
// 	if (deep==5)
// 	{
// 		return 1;
...
// 	for (int i = 0; i < 4; ++i)
// 	{
// 		if ((x+dx[i])>=4||(x+dx[i])<0) continue;
// 		if ((y+dy[i])>=3||(y+dy[i])<0) continue;
// 		if ((mp[x+dx[i]][y+dy[i]])&&!s[x+dx[i]][y+dy[i]])
// 		{
// 			s[x+dx[i]][y+dy[i]] = 1;
// 			return checkDfs(x+dx[i],y+dy[i],mp,deep+1);
// 		}
// 	}
// 	return 0;
// }
...
bool check(int j[])
{
	...
	while(i<iq)
	{
		...
		for (int k = 0; k < 4; ++k)
		{
			...
			if ((mp[queue[i].y+dy[k]][queue[i].x+dx[k]])&&(!s[queue[i].y+dy[k]][queue[i].x+dx[k]]))
			{
				s[queue[i].y+dy[k]][queue[i].x+dx[k]] = 1;
