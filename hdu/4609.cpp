...
#include <algorithm>
#define maxn 100007
typedef std::complex<double> complex_t; int n, bit_length;
...
fft(m, buffer, offset + step, step << 1, epsilon); for(int k = 0; k != m; ++k)
{
int pos = 2 * step * k;
temp[k] = buffer[pos + offset] + epsilon[k * step] * buffer[pos + offset + step]; temp[k + m] = buffer[pos + offset] - epsilon[k * step] * buffer[pos + offset + step];
}
for(int i = 0; i != n; ++i)
...
{
double pi = acos(-1); for(int i = 0; i != n; ++i) {
...
init_epsilon(n);
for (int i = 0; i < n; ++i)
A[i] = complex_t(i, 0); fft(n, A, 0, 1, epsilon); for (int i = 0; i < n; ++i)
        A[i] = complex_t(i, 0);
